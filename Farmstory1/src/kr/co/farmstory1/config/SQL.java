...
public class SQL {
	...
	// 게시판 관련
	public final static String SELECT_LATEST_ARTICLE = "(SELECT `seq`, `title`, `rdate` FROM `JBOARD_ARTICLE` WHERE `cate`='grow' AND `parent`=0 "
														+ "ORDER BY `seq` DESC LIMIT 5) "
														+ "UNION "
														+ "(SELECT `seq`, `title`, `rdate` FROM `JBOARD_ARTICLE` WHERE `cate`='school' AND `parent`=0 "
														+ "ORDER BY `seq` DESC LIMIT 5) "
														+ "UNION "
														+ "(SELECT `seq`, `title`, `rdate` FROM `JBOARD_ARTICLE` WHERE `cate`='croptalk' AND `parent`=0 "
														+ "ORDER BY `seq` DESC LIMIT 5)";
	...
	public final static String UPDATE_COMMENT_DELETE = "UPDATE `JBOARD_ARTICLE` SET `comment` = `comment` - 1 "
														+ "WHERE `seq`=?";
}
